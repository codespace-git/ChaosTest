...
class AgentDataBaseManager:
        ...
        with self.get_network_connection() as conn:
            conn.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS NETWORK (
                    ...
                    inject_prev INTEGER DEFAULT 0,
                    inject_next INTEGER DEFAULT 0,
    ...
    def get_network_state(self):
        ...
        with self.get_network_connection() as conn: 
            return conn.execute("SELECT prompt_attempt, start FROM NETWORK WHERE id = 1").fetchone()
    ...
    def get_inject_state(self):
        with self.get_network_connection() as conn:
            return conn.execute("SELECT inject_prev, inject_next, fail_count FROM NETWORK WHERE id = 1").fetchone()
        
    
    def update_inject_state(self,prev,succ,count):
