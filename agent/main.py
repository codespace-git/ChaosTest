...
t = default.default_items(items)
for i, item in enumerate(items):
    globals()[item] = t[i]


try:
    package = loader.load_config()
    if not package:
        ...
        try:
            ...
            try:
                t = loader.config_init(config,items)
                for i, item in enumerate(items):
                    globals()[item] = t[i]
...
for interval in intervals.keys():
    globals()[interval.upper()] = intervals[interval] 


inject_prev = c.InjectPrev
inject_next = c.InjectNext
fail_count = c.FailCount

special_strings = c.SpecialStrings

TOXIC_PROB = c.TOXICPROB
TOOL_LIMIT = c.TOOLLIMIT
PROMPT_LIMIT = c.PROMPTLIMIT
ERROR_PROB = c.ERRORPROB
...
def update_inject_with_smart_wait(db_manager: AgentDataBaseManager, prob: float):
    ...
    if fail_count == 0:
        ...
        db_manager.update_control_inject(inject_next)
...
def call_service_directly(tool_name, endpoint, method, params=None, json_data=None,attempts = 0,prompt_attempt = 0):
    ...
    while attempts < TOOL_LIMIT:
        ...
        try:
            ...
            log_direct_service_call(info_logger,tool_name, endpoint, direct_url, attempts, prompt_attempt+1, "success")
            ...
            if isinstance(e, requests.exceptions.Timeout):
                log_direct_service_call(info_logger,tool_name, endpoint, direct_url, attempts-1, prompt_attempt+1, "timeout")
            elif isinstance(e,requests.exceptions.ConnectionError):
               log_direct_service_call(info_logger,tool_name, endpoint, direct_url, attempts-1, prompt_attempt+1, "connection_error")
            continue
...
def call_with_toxic(tool_name, endpoint, method="GET", params=None, json_data=None):
    ...
    url = f"http://toxiproxy:{proxy[tool_name]}{endpoint}"
...
def response_handler(agent_executor: AgentExecutor, prompt: str, prob:float,db_manager:AgentDataBaseManager):
    state = True
    row = db_manager.get_network_state()
    attempt_no = row[0] + 1 if row else 1
